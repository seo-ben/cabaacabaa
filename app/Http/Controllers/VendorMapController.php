...
class VendorMapController extends Controller
{
    ...
    public function getNearbyVendors(Request $request)
    {
        ...
        // Grille de coordonnées pour la clé de cache (~1.1km de précision)
        $latGrid = round($userLat, 2);
        $lngGrid = round($userLng, 2);
        $cacheKey = "nearby_vendors_{$latGrid}_{$lngGrid}_{$radius}";

        $cacheRepository = \Illuminate\Support\Facades\Cache::supportsTags() 
            ? \Illuminate\Support\Facades\Cache::tags(['nearby_vendors']) 
            : \Illuminate\Support\Facades\Cache::store();

        $nearbyVendors = $cacheRepository->remember($cacheKey, 300, function () use ($userLat, $userLng, $radius) {
            ...
            return $vendeurs->map(function ($vendeur) use ($userLat, $userLng) {
            ...
            ->map(function ($vendeur) {
                ...
                $specialties = array_values($vendeur->categories->pluck('nom_categorie', 'nom_categorie')->toArray());
                ...
                return [
                    ...
                    'distance_text' => $vendeur->distance < 1 
                        ...
                        : $vendeur->distance . ' km',
                    'image' => $vendeur->thumbnail_url,
                    'image_full' => $vendeur->image_url,
                    ...
                    'products' => array_values($vendeur->plats->pluck('nom_plat', 'nom_plat')->toArray()),
