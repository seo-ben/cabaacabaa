...
<div class="max-w-3xl mx-auto">
    <div class="mb-10">
        ...
        <h1 class="text-3xl font-black text-gray-900 tracking-tight mt-4">Modifier : {{ $category->name }}</h1>
    </div>

    <form action="{{ route('admin.vendor-categories.update', $category->id_category_vendeur) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm space-y-8">
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" rows="4"
                          class="w-full px-8 py-5 bg-gray-50 border-0 rounded-[2rem] focus:bg-white focus:ring-4 focus:ring-orange-500/10 outline-none transition-all text-sm font-bold text-gray-900 resize-none">{{ old('description', $category->description) }}</textarea>
