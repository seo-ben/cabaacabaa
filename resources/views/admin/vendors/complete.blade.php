...
<div class="max-w-6xl mx-auto" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5">
        <div>
            <h1 class="text-xl font-black text-gray-900 tracking-tight">Configuration Commerciale</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <button form="enrichment-form" type="submit" class="px-5 py-2.5 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition shadow-lg shadow-red-100 flex items-center gap-2">
    ...
    <div class="flex overflow-x-auto gap-2 mb-6 p-1 bg-gray-100 rounded-lg w-fit border border-gray-200/50">
        <button @click="tab = 'infos'" :class="tab === 'infos' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            1. Informations
        </button>
        <button @click="tab = 'contacts'" :class="tab === 'contacts' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            2. Contacts
        </button>
        <button @click="tab = 'horaires'" :class="tab === 'horaires' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            3. Horaires
        </button>
        <button @click="tab = 'sections'" :class="tab === 'sections' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            4. Modules CMS
        </button>
        <button @click="tab = 'media'" :class="tab === 'media' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            5. Galerie
        </button>
        <button @click="tab = 'menu'" :class="tab === 'menu' ? 'bg-white text-red-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'" class="px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200">
            6. Mon Menu
        </button>
    </div>
    ...
    <form id="enrichment-form" action="{{ route('admin.vendors.store-enrichment', $user->id_user) }}" method="POST" enctype="multipart/form-data">
        ...
        <div x-show="tab === 'infos'" x-transition class="space-y-6">
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="nom_commercial" value="{{ old('nom_commercial', $user->nom_complet) }}" required 
                               class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:bg-white outline-none transition-all font-bold text-gray-900">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Type de Boutique / Catégorie</label>
                        <select name="id_category_vendeur" required class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:bg-white outline-none transition-all font-bold text-gray-900">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:bg-white outline-none transition-all font-medium text-gray-700" placeholder="Racontez votre histoire..."></textarea>
                </div>
                <div>
                    <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Zone de Service</label>
                    <select name="id_zone" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:bg-white outline-none transition-all font-bold text-gray-900">
        ...
        <div x-show="tab === 'contacts'" x-transition class="space-y-6">
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="space-y-6">
        ...
        <div x-show="tab === 'horaires'" x-transition class="space-y-6">
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <table class="w-full">
                    <thead>
        ...
        <div x-show="tab === 'sections'" x-transition class="space-y-6">
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div x-show="sections.length === 0" class="py-12 text-center text-gray-400 border-2 border-dashed border-gray-100 rounded-xl font-medium">
            ...
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-8">
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="p-4 border-2 border-dashed border-gray-200 rounded-xl text-center space-y-4">
                            <div class="w-12 h-12 bg-red-50 text-red-600 rounded-lg flex items-center justify-center mx-auto">
                        ...
                        <div class="p-4 border-2 border-dashed border-gray-200 rounded-xl text-center space-y-4">
                            <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center mx-auto">
        ...
        <div x-show="tab === 'menu'" x-transition class="space-y-6 text-left">
            <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-6">
                <div>
                    ...
                    <p class="text-gray-500 font-medium mb-5 italic text-sm">Sélectionnez les catégories de plats que ce vendeur est autorisé à publier. Il ne pourra pas ajouter de plats hors de cette sélection.</p>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($categories as $category)
                        <label class="group relative flex items-center gap-4 p-4 rounded-lg border border-gray-100 hover:border-red-200 hover:bg-red-50 transition-all cursor-pointer">
                ...
                @if(count($categories) === 0)
                    <div class="py-12 text-center text-gray-400 border-2 border-dashed border-gray-100 rounded-xl font-medium">
