...
@section('content')
<div x-data="{ showAddModal: false, searchQuery: '{{ request('search') }}', statusFilter: '{{ request('status') }}' }" @keydown.escape="showAddModal = false" class="space-y-4">
    <!-- Header Section -->
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="bg-white p-3.5 rounded-2xl border border-gray-100 shadow-sm">
            <p class="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Total</p>
            <p class="text-xl font-black text-gray-900 leading-none">{{ $stats['total'] }}</p>
        </div>

        <div class="bg-white p-3.5 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
            <p class="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-1.5">Actifs</p>
        ...
        <div class="bg-white p-3.5 rounded-2xl border border-gray-100 shadow-sm relative">
        ...
        <div class="bg-white p-3.5 rounded-2xl border border-gray-100 shadow-sm">
            <p class="text-[8px] font-black text-rose-500 uppercase tracking-widest mb-1.5">Suspendus</p>
    ...
    <div class="bg-white p-1.5 rounded-2xl border border-gray-100 shadow-sm">
        <form action="{{ route('admin.vendors.index') }}" method="GET" class="flex flex-col lg:flex-row lg:items-center gap-1">
            ...
            <div class="flex items-center gap-1.5 p-1.5">
                <button type="submit" class="px-5 py-2.5 bg-gray-900 text-white rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-gray-900/10">Filtrer</button>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        ...
                        <th class="hidden lg:table-cell px-3 py-3 font-black text-gray-400 uppercase tracking-widest leading-none">État</th>
                    ...
                    <tr class="group hover:bg-gray-50/50 transition-all duration-200">
                        <td class="pl-4 pr-3 py-2.5">
                            <div class="flex items-center gap-2.5">
                                <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 font-black text-xs group-hover:bg-purple-600 group-hover:text-white transition-all shadow-inner">
                                    {{ substr($v->nom_commercial, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="font-black text-gray-900 truncate">{{ $v->nom_commercial }}</p>
                                    <p class="text-[8px] text-gray-400 font-bold uppercase truncate">{{ $v->user->name ?? 'N/A' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="hidden sm:table-cell px-3 py-2.5">
                            <span class="inline-flex px-1.5 py-0.5 rounded-md bg-blue-50 text-blue-600 font-black text-[8px] uppercase tracking-tighter">
                                {{ $v->category ? $v->category->name : ($v->type_vendeur ?? 'AUTRE') }}
                            </span>
                            <p class="text-[7px] text-gray-300 font-bold uppercase mt-1 italic">{{ $v->zone->nom ?? 'Hors zone' }}</p>
                        </td>
                        <td class="px-3 py-2.5">
                            @php
                                $statusMap = [
                                    'verifie' => ['text-emerald-600', 'CERTIFIÉ'],
                                    'en_cours' => ['text-orange-500', 'EN ATTENTE'],
                                    'non_verifie' => ['text-amber-500', 'INCOMPLET'],
                                    'suspendu' => ['text-rose-600', 'SUSPENDU'],
                                ];
                                $st = $statusMap[$v->statut_verification] ?? ['text-gray-400', 'INCONNU'];
                            @endphp
                            <span class="font-black {{ $st[0] }} tracking-tighter">{{ $st[1] }}</span>
                        </td>
                        <td class="hidden lg:table-cell px-3 py-2.5 text-[8px]">
                            <div class="flex items-center gap-1.5">
                                <span class="w-1.5 h-1.5 rounded-full {{ $v->actif ? 'bg-emerald-500' : 'bg-gray-200' }}"></span>
                                <span class="font-bold {{ $v->actif ? 'text-gray-900' : 'text-gray-400' }} uppercase">{{ $v->actif ? 'En ligne' : 'Off' }}</span>
                            </div>
                        </td>
                        <td class="pr-4 pl-3 py-2.5 text-right">
                            <div class="flex items-center justify-end gap-1.5">
                                <a href="{{ route('admin.vendors.show', $v->id_vendeur) }}" class="p-2 bg-gray-50 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all" title="Détails">
...
<!-- Add Modal Simplified Padding -->
<div x-show="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4" x-cloak>
    <div @click.away="showAddModal = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col">
        <div class="px-5 py-4 bg-gray-900 text-white flex items-center justify-between">
            <h2 class="text-sm font-black uppercase tracking-widest">Inscrire un partenaire</h2>
            <button @click="showAddModal = false" class="text-gray-400 hover:text-white transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form action="{{ route('admin.vendors.store') }}" method="POST" class="p-6 space-y-4 overflow-y-auto">
            @csrf
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Nom Responsable</label>
                    <input type="text" name="name" required class="w-full px-3 py-2 bg-gray-50 border-none rounded-xl text-xs font-bold text-gray-900 focus:bg-white focus:ring-2 focus:ring-purple-500/10 transition-all">
                </div>
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Email Personnel</label>
                    <input type="email" name="email" required class="w-full px-3 py-2 bg-gray-50 border-none rounded-xl text-xs font-bold text-gray-900 focus:bg-white focus:ring-2 focus:ring-purple-500/10 transition-all">
                </div>
            </div>
            <div class="space-y-1.5">
                <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Nom Commercial (Boutique)</label>
                <input type="text" name="nom_commercial" required class="w-full px-3 py-2 bg-gray-50 border-none rounded-xl text-xs font-bold text-gray-900 focus:bg-white focus:ring-2 focus:ring-purple-500/10 transition-all">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Catégorie</label>
                    <select name="id_category_vendeur" required class="w-full px-3 py-2 bg-gray-50 border-none rounded-xl text-xs font-bold text-gray-900 focus:bg-white focus:ring-2 focus:ring-purple-500/10 transition-all">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="id_zone" class="w-full px-3 py-2 bg-gray-50 border-none rounded-xl text-xs font-bold text-gray-900 focus:bg-white focus:ring-2 focus:ring-purple-500/10 transition-all">
            ...
            <div class="flex gap-3 pt-4">
                <button type="submit" class="flex-1 py-3 bg-gray-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-gray-900/10">Valider</button>
