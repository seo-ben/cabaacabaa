...
<main class="block lg:hidden bg-gray-50 dark:bg-slate-950 min-h-screen font-sans pb-24">
    ...
    <div class="sticky top-0 z-30 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-gray-100 dark:border-slate-800/50 px-4 py-4">
        <h1 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Mes Commandes</h1>
        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">{{ $commandes->total() }} commandes au total</p>
    </div>
    ...
    @else
    {{-- Orders List --}}
    <section class="px-4 pt-4 space-y-4">
        @foreach($commandes as $commande)
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm overflow-hidden">
            {{-- Header row --}}
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-50 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/30">
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 bg-gray-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-gray-500 dark:text-gray-400">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-gray-700 dark:text-gray-300">{{ $commande->date_commande->format('d M Y') }}</p>
                    </div>
                </div>
                <span class="px-3 py-1.5 text-[8px] font-black uppercase tracking-widest rounded-xl border
                    @if($commande->statut == 'en_attente') bg-orange-50 dark:bg-orange-900/20 text-orange-600 border-orange-100 dark:border-orange-900/30
                    @elseif($commande->statut == 'confirmee' || $commande->statut == 'terminee') bg-green-50 dark:bg-green-900/20 text-green-600 border-green-100 dark:border-green-900/30
                    @elseif($commande->statut == 'annulee') bg-red-50 dark:bg-red-900/20 text-red-500 border-red-100 dark:border-red-900/30
                    @else bg-blue-50 dark:bg-blue-900/20 text-blue-600 border-blue-100 dark:border-blue-900/30 @endif">
            ...
            <div class="px-4 py-3">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center gap-2">
                        <div class="w-7 h-7 bg-orange-100 dark:bg-orange-900/30 rounded-lg flex items-center justify-center font-black text-[10px] text-orange-600 dark:text-orange-400">
                    ...
                    <span class="text-base font-black text-orange-600 dark:text-orange-400">{{ number_format($commande->montant_total, 0) }} <small class="text-[9px]">FCFA</small></span>
                </div>

                <div class="flex items-center gap-2 mb-4">
                    <span class="text-[9px] font-black uppercase tracking-widest px-2 py-1 bg-gray-100 dark:bg-slate-800 text-gray-500 rounded-lg flex items-center gap-1">
                        ...
                        @endif
                        {{ $commande->type_recuperation }}
                    </span>
                    @if($commande->paiement_effectue)
                    <span class="text-[9px] font-black uppercase tracking-widest px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-600 rounded-lg flex items-center gap-1">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('order.confirmation', $commande->id_commande) }}" class="flex-1 py-3 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 text-[10px] font-black uppercase tracking-widest rounded-xl text-center active:scale-95 transition-transform hover:bg-orange-50 dark:hover:bg-orange-900/20 hover:text-orange-600">
                        Suivre
                    </a>
                    <a href="{{ route('orders.reorder', $commande->id_commande) }}" class="flex-1 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[10px] font-black uppercase tracking-widest rounded-xl text-center active:scale-95 transition-transform hover:bg-orange-600">
                        Réitérer
                    </a>
                    @if($commande->statut == 'en_attente')
                    <a href="{{ route('orders.cancel', $commande->id_commande) }}"
                       onclick="return confirm('Annuler cette commande ?')"
                       class="py-3 px-3 bg-red-50 dark:bg-red-900/20 text-red-500 text-[10px] font-black uppercase tracking-widest rounded-xl active:scale-95 transition-transform">
    ...
    <div class="px-4 mt-6 flex justify-center scale-90">
        {{ $commandes->links('vendor.pagination.premium') }}
    </div>
    @endif
                        ...
                        <div class="flex flex-col lg:flex-row">
                            <div class="flex-1 p-10 lg:p-12 space-y-8">
                                <div class="flex flex-wrap items-center justify-between gap-6">
                                    <div class="flex items-center gap-4">
                                        <div class="w-14 h-14 bg-gray-50 dark:bg-gray-800 rounded-2xl flex items-center justify-center text-gray-400 group-hover:bg-orange-50 dark:group-hover:bg-orange-900/20 group-hover:text-orange-600 transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @if($commande->paiement_effectue)
                                            <span class="px-5 py-2.5 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 border border-current rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg> Payé
                                            </span>
                                        @endif
                                    </div>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-3 gap-10 pt-8 border-t border-gray-50 dark:border-gray-800">
                                    <div>
                                        <h4 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-3">Établissement</h4>
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 bg-gray-50 dark:bg-gray-800 rounded-lg flex items-center justify-center text-orange-600 font-black italic text-xs">{{ substr($commande->vendeur->nom_commercial, 0, 1) }}</div>
                                            <span class="text-sm font-black text-gray-900 dark:text-white">{{ $commande->vendeur->nom_commercial }}</span>
                                        </div>
                                    </div>
                                    <div>
                            ...
                            <div class="bg-gray-50 dark:bg-gray-800 lg:w-72 p-10 flex flex-col justify-center gap-4 border-l dark:border-gray-800">
                                <a href="{{ route('order.confirmation', $commande->id_commande) }}" class="w-full py-4 bg-white dark:bg-gray-700 border-2 border-transparent hover:border-orange-500 hover:text-orange-600 text-gray-900 dark:text-white text-[10px] font-black uppercase tracking-widest rounded-2xl text-center transition-all shadow-sm">Suivre la commande</a>
                                @if($commande->statut == 'en_attente')
                                    <a href="{{ route('orders.cancel', $commande->id_commande) }}" onclick="return confirm('Voulez-vous vraiment annuler cette commande ?')" class="w-full py-4 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-[10px] font-black uppercase tracking-widest rounded-2xl text-center hover:bg-red-600 hover:text-white transition-all">Annuler</a>
                                @endif
                                <a href="{{ route('orders.reorder', $commande->id_commande) }}" class="w-full py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[10px] font-black uppercase tracking-widest rounded-2xl text-center hover:bg-orange-600 transition-all shadow-xl active:scale-95">Commander à nouveau</a>
                            </div>
                        </div>
                    </div>
                @endforeach
